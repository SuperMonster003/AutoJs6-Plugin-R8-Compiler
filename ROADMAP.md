...
- [x] 文档一致性自检: `python .python/generate_markdown.py --check` 以只读方式按 UTF-8 字节核对全部 26 个生成文件, 缺失或漂移即非零退出; 已完成正向与临时漂移负向验证. (2026-08-27)
- [x] keep 规则入门指南: 新增英文与简体中文实用指南, 覆盖 `Packages`/反射访问, JNI 方法与描述符类型, Java 序列化, 公共 API 面, 协议 1.0 白名单及排障收窄流程, 并从 10 种语言 README 链接. (2026-08-27)
- [x] 应用内说明: 新增只读启动器界面, 展示插件/R8 版本、服务组件可用状态与内置本地化更新日志; 完成英语、简繁中文、法语、西班牙语、日语、韩语、俄语与阿拉伯语共 10 种界面语言, 且明确 provider 仍须在 AutoJs6 开发者选项中启用. (2026-08-27)
