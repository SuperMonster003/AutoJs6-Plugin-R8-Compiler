...
def check(root, pending_commit=False):
    ...
    import hashlib
    provenance = json.loads((root/'libs/common-plugin-api.provenance.json').read_text(encoding='utf-8'))
    artifact = root/provenance['file']
    require(artifact.is_file(), 'Pinned common plugin API artifact is missing')
    if artifact.is_file():
        require(hashlib.sha256(artifact.read_bytes()).hexdigest() == provenance['sha256'], 'Pinned common plugin API SHA-256 differs')
    return errors
